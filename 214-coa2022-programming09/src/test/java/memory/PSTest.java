...
public class PSTest {
    ...
    public void test2() {
        ...
        byte[] expect = helper.fillData('8', len);
        byte[] actual = mmu.read("000000000000000000000000000000000010000000000000", len);
        assertArrayEquals(expect, actual);
    ...
    public void test3() {
        ...
        for (int i = 0; i < 4 * 3; i++) {
            Arrays.fill(expect, i * 1024, i * 1024 + 1024, (byte) (0x40 + i));
        }
        byte[] actual = mmu.read("000000000000000000000000000000000100000000000000", len);
        assertArrayEquals(expect, actual);
    ...
    public void test4() {
        ...
        byte[] expect = new byte[len];
        for (int i = 0; i < 4; i++) {
            Arrays.fill(expect, i * 1024, i * 1024 + 1024, (byte) (0x52 + i));
        }
        byte[] actual = mmu.read("000000000000000000000000000000011000100000000000", len);
        assertArrayEquals(expect, actual);
