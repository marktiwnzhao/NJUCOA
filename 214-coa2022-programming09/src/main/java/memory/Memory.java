...
public class Memory {
    ...
    public void real_load(String pAddr, int len) {
        // TODO
        write(pAddr, len, disk.read(pAddr, len));
    }
    ...
    public void seg_load(int segIndex) {
        ...
        Arrays.fill(segDescriptor.base, '0');
        Arrays.fill(segDescriptor.limit, '1');
        segDescriptor.validBit = true;
        if(PAGE) {
            segDescriptor.granularity = true;
        } else {
            int len = Integer.parseInt(Transformer.binaryToInt(String.valueOf(segDescriptor.limit)));
            String addr = String.valueOf(segDescriptor.base);
            write("0", len, disk.read(addr, len));
        }
    }
    ...
    public void page_load(int vPageNo) {
        // TODO
        String addr = Transformer.intToBinary(String.valueOf(vPageNo*PAGE_SIZE_B));
        int pageFrame = 0;
        ...
        pageValid[pageFrame] = true;
        PageItem pageItem = getPageItem(vPageNo);
        pageItem.pageFrame = Transformer.intToBinary(String.valueOf(pageFrame)).substring(12).toCharArray();
        pageItem.isInMem = true;
        String pAddr = Transformer.intToBinary(String.valueOf(pageFrame*PAGE_SIZE_B));
        write(pAddr, PAGE_SIZE_B, disk.read(addr, PAGE_SIZE_B));
    }
