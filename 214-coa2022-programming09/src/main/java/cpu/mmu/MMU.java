...
public class MMU {
    ...
    public byte[] read(String logicAddr, int length) {
        ...
        // TODO: add cache here
        if(Cache.isAvailable) return cache.read(physicalAddr, length);
        else return memory.read(physicalAddr, length);
    }
    ...
    public void write(String logicAddr, int length, byte[] data) {
        String physicalAddr = addressTranslation(logicAddr, length);
        // TODO: add cache here
        if(Cache.isAvailable) cache.write(physicalAddr, length, data);
        else memory.write(physicalAddr, length, data);
    }
        ...
        if (!Memory.SEGMENT) {
            ...
            if (!Memory.PAGE) {
                ...
                for (int i = startvPageNo; i <= endvPageNo; i++) {
                    // TODO: add tlb here
                    if(TLB.isAvailable && tlb.isValidPage(i))
                        continue;
    ...
    private String toRealLinearAddr(String logicAddr) {
        // TODO
        String SegmentReg = logicAddr.substring(0, 16) + "0000";//左移4位
        String offest = logicAddr.substring(32);//低16位
        SegmentReg = "000000000000" + SegmentReg;
        offest = "0000000000000000" + offest;
        ALU alu = new ALU();
        return alu.add(new DataType(SegmentReg), new DataType(offest)).toString();
    }
    ...
    private String toSegLinearAddr(String logicAddr) {
        ...
        String base = String.valueOf(memory.getBaseOfSegDes(segIndex));
        ALU alu = new ALU();
        return alu.add(new DataType(base), new DataType(offset)).toString();
    }
    ...
    private String toPagePhysicalAddr(String linearAddr) {
        ...
        String pageFrame;
        if(TLB.isAvailable && tlb.isValidPage(vPageNo)) {
            pageFrame = String.valueOf(tlb.getFrameOfPage(vPageNo));
        } else pageFrame = String.valueOf(memory.getFrameOfPage(vPageNo));
        return pageFrame + linearAddr.substring(20, 32);
    }
